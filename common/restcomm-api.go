...
func (api RestcommApi) MakeCall(from string, to string, callback string) (*CallInfo, error) {
	return api.MakeCallWithStatus(from, to, callback, "")
}

func (api RestcommApi) MakeCallWithStatus(from string, to string, callback string, statusCallback string) (*CallInfo, error) {
	fmt.Println("\tapi.MakeCall: from =", from, " to =", to, " callback =", callback, "statusCallback", statusCallback)
	acc := api.User + ":" + api.Pass
...
func (api *RestcommApi) GetRecording(sid string) (string, error) {
	acc := api.User + ":" + api.Pass
	path := fmt.Sprintf("http://%s@%s/restcomm/2012-04-24/Accounts/%s/Calls/%s/Recordings.json", acc, api.Server, api.User, sid)

	Trace.Println("get recordings: ", path)
	resp, err := http.Get(path)
	if err != nil {
		return "", err
	}
	...
	Trace.Println("Recordings", string(body))

	jr := make(map[string]interface{})
	json.Unmarshal(body, &jr)

	Trace.Println("json map", jr)
	return jr["Sid"].(string), nil
}

func (api *RestcommApi) SendSms(to string, msg string) error {
	Trace.Println("Send sms to", to)
	return nil
}
